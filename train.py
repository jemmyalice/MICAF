...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            has_path = any(isinstance(item, Path) for item in results)
            if has_path:
                # 将 Path 对象转换为字符串
                filtered_vals = [str(v) if isinstance(v, Path) else v for v in vals]
                # 写入文件
                n = len(filtered_vals) + 1  # number of cols
                s = '' if file.exists() else (('%s,' * n % tuple(['epoch'] + keys)).rstrip(',') + '\n')  # add header
                with open(file, 'a') as f:
                    f.write(s + ('%s,' * n % tuple([epoch] + filtered_vals)).rstrip(',') + '\n')
            else:
                n = len(dicts) + 1  # number of cols
                s = '' if file.exists() else (('%s,' * n % tuple(['epoch'] + keys)).rstrip(',') + '\n')  # add header
                with open(file, 'a') as f:
            ...
            has_path = any(isinstance(item, Path) for item in results)
            if has_path:
                # 用 0 替换 Path 对象，保持列数不变
                filtered_results = list(results)  # 将元组转换为列表以便修改
                filtered_results[3] = 0.0  # 替换为 float 类型的 0.0
                filtered_results = tuple(filtered_results)
                # 将过滤后的结果转换为 NumPy 数组
                x = np.array(filtered_results).reshape(1, -1)
                fi = fitness(x)  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
            else:
                fi = fitness(np.array(results).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
    ...
    if rank in [-1, 0]:
        ...
        for m in (last, best) if best.exists() else (last):  # speed, mAP tests
            model = attempt_load(m, device)  # 加载模型，不要使用 .half()
            model.eval()  # 将模型设置为评估模式

            with torch.no_grad(), amp.autocast(enabled=cuda):  # 使用 autocast 包裹测试过程
                results, _, MRresult, _ = test.test(
                    opt.data,
                    batch_size=1,
                    imgsz=imgsz_test,
                    conf_thres=0.001,
                    iou_thres=0.5,
                    model=model,  # 使用未转换的模型
                    single_cls=opt.single_cls,
                    dataloader=testloader,
                    save_dir=save_dir,
                    save_txt=True,
...
if __name__ == '__main__':
    ...
    if not os.path.exists(opt.weights):
        ...
        # 方法 1：使用 torch.hub 下载
        torch.hub.download_url_to_file(
            "https://github.com/ultralytics/assets/releases/download/v8.2.0/yolov5lu.pt",  # 权重文件 URL
            opt.weights  # 保存路径
        )
